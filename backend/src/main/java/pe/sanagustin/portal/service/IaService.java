...
import pe.sanagustin.portal.repository.PlanApoyoRepository;
...
public class IaService {
    ...
    private final PlanApoyoRepository planApoyoRepository;

    @PersistenceContext
    private EntityManager em;

    @Async("academicExecutor")
    public CompletableFuture<Map<String, String>> obtenerRecomendaciones(String codigoDocente, Integer idAlumno, double asistencia, double promedio, String causas) {
        long idMaestro = 1L;
        try {
            var maestro = em.createQuery(
                    "SELECT m FROM Maestro m WHERE m.usuario.codigo = :codigo", Maestro.class)
                    .setParameter("codigo", codigoDocente)
                    .getSingleResult();
            idMaestro = maestro.getIdMaestro();
        } catch (Exception e) {
            log.warn("IaService: No se encontró maestro para código {}, usando fallback idMaestro=1", codigoDocente);
        }
        ...
        try {
            ...
            log.warn("IaService: No se encontró padre para idAlumno {}, usando fallback idPadre=1", idAlumno);
        }

        // Buscar plan existente
        Optional<PlanApoyo> planOpt = planApoyoRepository.findByIdAlumnoAndIdMaestro(idAlumno, (int) idMaestro);
        if (planOpt.isPresent()) {
            PlanApoyo plan = planOpt.get();
            // Si las métricas coinciden, cargamos de la BD sin llamar a la IA
            if (Math.abs(plan.getAsistenciaReg() - asistencia) < 0.01 && Math.abs(plan.getPromedioReg() - promedio) < 0.01) {
                return CompletableFuture.completedFuture(Map.of(
                        "resultado", plan.getPlanJson(),
                        "checksState", plan.getChecksState(),
                        "feedback_1", plan.getFeedback1() != null ? plan.getFeedback1() : "",
                        "feedback_2", plan.getFeedback2() != null ? plan.getFeedback2() : "",
                        "feedback_3", plan.getFeedback3() != null ? plan.getFeedback3() : ""
                ));
            } else {
                // Si cambiaron, eliminamos el plan anterior para regenerarlo
                planApoyoRepository.delete(plan);
                em.flush();
            }
        }
        ...
        if (promedio >= 11.0 && asistencia >= 70.0) {
            return CompletableFuture.completedFuture(Map.of("resultado", "{\"error\": \"El alumno no requiere plan de apoyo en este momento.\"}"));
        }

        String nombreAlumno = "el estudiante";
        try {
            var al = em.find(Alumno.class, idAlumno.longValue());
            if (al != null) {
                nombreAlumno = al.getNombre() + " " + al.getApellido();
            }
        } catch (Exception e) {
            log.error("IaService: Error al buscar Alumno {}", idAlumno, e);
        }
        ...
        try {
            PlanApoyo plan = new PlanApoyo();
            plan.setIdAlumno(idAlumno);
            plan.setIdMaestro((int) idMaestro);
            plan.setPlanJson(consejo);
            plan.setChecksState("0,0,0");
            plan.setAsistenciaReg(asistencia);
            plan.setPromedioReg(promedio);
            planApoyoRepository.save(plan);
        } catch (Exception e) {
            log.error("IaService: Error al guardar PlanApoyo en base de datos", e);
        }
    ...
    @Async("academicExecutor")
    public CompletableFuture<Map<String, String>> guardarFeedbackPlan(String codigoDocente, FeedbackPlanRequest request) {
        Integer idAlumno = request.idAlumno();
        Integer checkIndex = request.checkIndex();
        ...
        Optional<PlanApoyo> planOpt = planApoyoRepository.findByIdAlumnoAndIdMaestro(idAlumno, (int) idMaestro);
        if (planOpt.isEmpty()) {
            throw new NoSuchElementException("No existe un plan activo para el alumno y maestro especificados.");
        }

        PlanApoyo plan = planOpt.get();
        String checksState = plan.getChecksState();
        String f1 = plan.getFeedback1();
        String f2 = plan.getFeedback2();
        String f3 = plan.getFeedback3();
        ...
        String userPrompt = String.format(
                ...
                plan.getPlanJson(), checkIndex + 1, feedback, idPadre);

        String planActualizado = openAiService.llamarOpenAi(systemPrompt, userPrompt, true);

        // Guardar en BD
        try {
            plan.setPlanJson(planActualizado);
            plan.setChecksState(nuevoChecksState);
            plan.setFeedback1(f1);
            plan.setFeedback2(f2);
            plan.setFeedback3(f3);
            planApoyoRepository.save(plan);
        } catch (Exception e) {
            log.error("IaService: Error al actualizar PlanApoyo en base de datos", e);
        }
    ...
    public String generarActaHTML(Integer idAlumno) {
        ...
        try {
            ...
            log.error("IaService: Error al buscar Alumno para acta", e);
        }

        String f1 = "", f2 = "", f3 = "";
        try {
            List<PlanApoyo> planes = em.createQuery(
                    "SELECT p FROM PlanApoyo p WHERE p.idAlumno = :idAlumno", PlanApoyo.class)
                    .setParameter("idAlumno", idAlumno)
                    .getResultList();
            if (!planes.isEmpty()) {
                PlanApoyo plan = planes.get(0);
                f1 = plan.getFeedback1();
                f2 = plan.getFeedback2();
                f3 = plan.getFeedback3();
            }
        } catch (Exception e) {
            log.error("IaService: Error al buscar feedback para acta", e);
        }
    ...
    @Async("academicExecutor")
    public CompletableFuture<String> redactarComoDocente(String codigoDocente, RedactarMensajeRequest request) {
        String borrador = request.texto();
        String nombreAlumno = request.nombreAlumno();
        String nombreDestinatario = request.nombreDestinatario();
        ...
        try {
            ...
            log.warn("IaService: No se encontró docente para código {}", codigoDocente);
        }
        ...
        return CompletableFuture.completedFuture(openAiService.llamarOpenAi(systemPrompt, userPrompt, false));
    }

    @Async("academicExecutor")
    public CompletableFuture<String> redactarComoPadre(String codigoPadre, RedactarPadreRequest request) {
        String borrador = request.texto();
        String nombreAlumno = request.nombreAlumno();
        String nombreDestinatario = request.nombreDestinatario();
        String relacion = request.relacion();

        String nombrePadre = "Apoderado";
        try {
            var padre = em.createQuery(
                    "SELECT p FROM Padre p WHERE p.usuario.codigo = :codigo", Padre.class)
                    .setParameter("codigo", codigoPadre)
                    .getSingleResult();
            nombrePadre = padre.getNombre() + " " + padre.getApellido();
        } catch (Exception e) {
            log.warn("IaService: No se encontró padre para código {}", codigoPadre);
        }
    ...
    @Async("academicExecutor")
    public CompletableFuture<String> analizarBiEscolar() {
    ...
    public List<Map<String, Object>> getAlertasEfectividad() {
        List<Map<String, Object>> alerts = new ArrayList<>();
        try {
            List<PlanApoyo> planesCompletos = planApoyoRepository.findByChecksState("1,1,1");
            for (PlanApoyo p : planesCompletos) {
                String studentName = "Alumno";
                String teacherName = "Maestro";

                var al = em.find(Alumno.class, p.getIdAlumno().longValue());
                if (al != null) studentName = al.getNombre() + " " + al.getApellido();

                var ma = em.find(Maestro.class, p.getIdMaestro().longValue());
                if (ma != null) teacherName = ma.getNombre() + " " + ma.getApellido();
            ...
            log.error("IaService: Error al recuperar alertas de efectividad", e);
        }
    ...
    @Async("academicExecutor")
    public CompletableFuture<Map<String, Object>> getTutorScores() {
        ...
        try {
            ...
            log.error("IaService: Error al calcular tutor scores", e);
        }
    ...
    public CompletableFuture<Map<String, String>> chatMaterial(ChatMaterialRequest request) {
        String idMaterialStr = request.idMaterial();
        ...
        return CompletableFuture.completedFuture(Map.of("respuesta", respuesta));
    }
}
